...
public class LongestUncommonSubsequenceI {
	/*
	Q: Given a group of two strings, you need to find the longest uncommon subsequence of 
	...
	*/
	...
	Algorithm: 
		...
		1. two strings are equal, no possibility for uncommon subsequence.
		2. two strings are not equal, but have same length, any one's whole can be
		the answer, so just return the length of the string.
		3. no equal, no equal length, just return the length of the longest string is
		ok since another one cannot equal to the longest one.

	Complexity Analysis:
	T: O(min(m, n)) equals will take min(m, n) time.
	S: O(1)
	*/
